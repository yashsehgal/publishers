  ...
  module Models
    class PeerToPeerRegistration < Client
      ...
      def create(publisher:, promo_campaign:)
        ...
        connection ||= begin
          require "faraday"
          Faraday.new(url: Rails.application.secrets[:api_promo_base_uri]) do |faraday|
        ...
        connection.send(:put) do |req|
          req.url(Rails.application.secrets[:api_promo_base_uri] + "api/2/promo/referral_code/p2p/#{publisher.owner_identifier}" + "?cap=1000")
